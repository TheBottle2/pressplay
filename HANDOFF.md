...
| 2026-09-11 | GitHub uçtan uca doğrulama (34/34) | `/tmp/verify_pressplay.sh` yazıldı: taze klon → 18 dosya kontrolü → rebrand tutarlılığı (paket/sürüm/desktop, eski-ad kalıntısı, script sözdizimi) → `cargo check` + `cargo test` (16/16) → klon içinde `cargo build --release` + binary kontrolü. Sonuç: 34 geçti, 0 kaldı. Script repo DIŞINDA (/tmp), istenirse eklenir |
